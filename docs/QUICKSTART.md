...
A window opens with a blinking cursor. This is where you'll run all the commands below. You can pin Terminal to your Dock: right-click its icon in the Dock → Options → Keep in Dock.
...
Same as macOS — this handles everything. Wait for the green "Chronicle Beta is ready!" banner.
...
Homebrew isn't in your PATH. Re-run the "Next steps" commands from the Homebrew installer, or run:
```bash
eval "$(/opt/homebrew/bin/brew shellenv)"
```

**python3 shows an old version after installing a new one**
The system Python is taking priority. On macOS, run:
```bash
echo 'export PATH="/opt/homebrew/bin:$PATH"' >> ~/.zshrc && source ~/.zshrc
```
On Linux, check `which python3` to see which one is active.

**Install script says "Python 3.10+ is required but not found"**
Same as above — the script can't find a recent Python in your PATH. Verify `python3 --version` shows 3.10+ before running install.sh.

**First run downloads ~2 GB**
sentence-transformers pulls PyTorch. This is a one-time download. If it times out, re-run the script — pip caches partial downloads.

**Indexing takes a long time**
Embedding 30k+ chunks runs locally on your CPU. It takes 5–45 minutes depending on your hardware. Apple Silicon Macs are typically fast (under 10 minutes). The script shows a progress bar.

**pip install fails with "[Errno 5] Input/output error" on WSL**
This is a known WSL + NTFS issue. Re-run the script — if dependencies were partially installed, it will detect them and skip the failing step.

**Claude Desktop says "Could not load app settings" after editing config**
Your JSON is malformed. Common mistakes: two separate JSON objects instead of one merged object, trailing commas, missing commas between keys, or the file was saved as Rich Text instead of plain text. Validate your JSON at [jsonlint.com](https://jsonlint.com/).

**health_check returns "degraded" with a path error**
The MCP server can't find the `data/vector_store` directory. This usually means the working directory isn't set correctly. Verify the config block from install.sh has the correct paths. On macOS/Linux, the config uses a bash wrapper that `cd`s into the project directory before launching the server — make sure the full `args` string is copied exactly.

**health_check returns "degraded" with "Read-only file system"**
ChromaDB needs write access to the vector store directory. Check permissions on `data/vector_store/`. On WSL, this can happen if `/mnt/c` is mounted read-only — check `mount | grep mnt/c`.

**Claude Desktop doesn't show Chronicle tools after config change**
Make sure you fully quit Claude Desktop (⌘+Q on macOS, right-click tray → Quit on Windows). Just closing the window doesn't restart the app. Also verify the config file path is correct — Claude Desktop may have changed it after an update.

**TextEdit (macOS) corrupts the JSON config file**
TextEdit defaults to Rich Text mode. Use `nano` in Terminal instead, or switch TextEdit to plain text first: Format → Make Plain Text.
